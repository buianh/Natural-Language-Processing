...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        if transition=="S":
            ...
            self.dependencies.append((self.stack[length-1],self.stack[length-2]))
            self.stack.pop(length-2)
        else:
            length = len(self.stack)
            self.dependencies.append((self.stack[length-2],self.stack[length-1]))
    ...
    def not_complete(self):
        return (len(self.buffer) != 0) or (len(self.stack) != 1)
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
       ...
       unfinished_parses[:] = [parse for parse in unfinished_parses if parse.not_complete()]
